    ...
    <script>
        $(document).ready(function(){
            ...
            $(".comment_form").submit(function(event){
                ...
                var vedio_id = '<?php echo $vedio->id; ?>';
                ...
                $.ajax( {
                    ...
                    success: function(data) {
                        $.get( "/get_speech_comments/"+vedio_id)
                                .done(function( data ) {
                                    console.log(data);
                                    ...
                                    for(var i=0;i<data.length;i++){
                                        if(data[i]["user_image"] == ""){
                                            $(".general_comment").append('<div style="background-color:#EEF4F5; padding:20px;height:200px; margin:25px;">' +
                                                    ...
                                                    '<div class="row replay_box" style="margin-top: 15px!important;"> ' +
                                                    '<form class="form-inline replay_form" role="form" style="text-align:right; padding-top: 2%; padding-bottom: 2%;">' +
                                                    '<input type="hidden" name="_token" value="'+token+'"><input type="hidden" name="article_id" value="'+data[i]["speech_id"]+'">' +
            ...
            $(".replay_form").submit(function (event) {
                ...
                $.ajax({
                    url: '/speech_comment_replay',
                    type: 'POST',
                    data: new FormData(this),
                    processData: false,
                    contentType: false,
                    success: function (data) {
                        $.get( "/get_speech_comment_replays/"+comment_id)
                                .done(function( data ) {
